  	...
  	module Ciba
	    class ConsentNotify < CommonBusinessRules
			...
			def notifyTheConsumptionApplication
				# TODO: call the rest interface configured in oauth_application - columns ciba_notify_type / ciba_notify_endpoint
				
				ciba_notify_type = @application.ciba_notify_type
				ciba_notify_endpoint = @application.ciba_notify_endpoint
				
				::Rails.logger.info("## ConsentNotify: notifyTheConsumptionApplication ## TODO ## => " + @auth_req_id.to_s + " application id:" + @application_id)
				::Rails.logger.info("## ConsentNotify: notifyTheConsumptionApplication ## TODO ## => ciba_notify_type: " + @ciba_notify_type.to_s + " ciba_notify_endpoint: " + ciba_notify_endpoint)
				::Rails.logger.info("## ConsentNotify: notifyTheConsumptionApplication ## TODO ## => client_notification_token (Bearer token): " + @authreq.client_notification_token.to_s)
